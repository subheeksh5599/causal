...
**The first real calendar write landed four and a half hours late, and Google rejected the obvious fix.** A hardcoded `timeZone: Europe/London` made a contract authorising `15:00` create the event at 15:00 London — 19:30 on the operator's calendar — and verification caught it: `start '2026-09-15T19:30' != authorised '2026-09-15T15:00'`. Dropping the `timeZone` key was worse: Google rejects the request body with `400 Missing time zone definition for start time`. The working form is the wall clock with the operator's own offset attached — and that is what the committed live run used.
